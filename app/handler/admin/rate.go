...
func (Rate) Sync(ctx *gin.Context) {
	if err := model.CoingeckoRate(); err != nil {
		base.BadRequest(ctx, "sync Coingecko failed: "+err.Error())
		return
	}
	if err := model.CmcRate(); err != nil {
		base.BadRequest(ctx, "sync CMC failed: "+err.Error())
		return
	}

	base.Ok(ctx, "sync done")
}
